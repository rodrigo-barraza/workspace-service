...
export class FileHandler {
    ...
    async fileInfo({ paths }) {
        ...
        const results = await Promise.all(pathList.map(async (p) => {
    ...
    async fileDiff({ pathA, pathB, content, contextLines = 3 }) {
        ...
        try {
            ...
            const diff = createTwoFilesPatch(validA.resolved, labelB, contentA, contentB, "", "", { context: Math.min(contextLines, 10) });
