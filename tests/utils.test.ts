...
// ══════════════════════════════════════════════════════════════
// Default Mode — No Virtualization (standalone / WSL)
// ══════════════════════════════════════════════════════════════
//
// When WORKSPACE_ACTUAL_ROOT is NOT set (or equals WORKSPACE_VIRTUAL_ROOT),
// virtualization is disabled. All paths pass through unchanged.

describe("Default Mode — No Virtualization", () => {

  beforeEach(() => {
    vi.resetModules();
    delete process.env.WORKSPACE_ACTUAL_ROOT;
    delete process.env.WORKSPACE_VIRTUAL_ROOT;
  });

  it("should disable virtualization when no env vars are set", async () => {
...
describe("validateWorkspacePath", () => {
  ...
  it("should resolve '.' to roots[0]", async () => {
  ...
  it("should resolve relative paths against roots[0]", async () => {
    ...
    const result = validateWorkspacePath("./src/index.ts", ["/home/rodrigo/development"]);

    expect(result.safe).toBe(true);
    expect(result.resolved).toBe("/home/rodrigo/development/src/index.ts");
  });

  it("should strip surrounding quotes from LLM-generated paths", async () => {
    const { validateWorkspacePath } = await import("../src/utils.ts");

    const result = validateWorkspacePath('"."', ["/home/rodrigo/development"]);
    expect(result.safe).toBe(true);
    expect(result.resolved).toBe("/home/rodrigo/development");
  });

  it("should reject empty or non-string input", async () => {
    const { validateWorkspacePath } = await import("../src/utils.ts");

    expect(validateWorkspacePath("", ["/root"]).safe).toBe(false);
...
// ══════════════════════════════════════════════════════════════
// Path Virtualization (Docker: LLM sees "/" → actual "/workspace")
// ══════════════════════════════════════════════════════════════
//
// These tests simulate Docker mode by setting WORKSPACE_ACTUAL_ROOT=/workspace
// via env var. Each test uses vi.resetModules() + fresh import to force the
// module to re-evaluate with the env var in place.

describe("Path Virtualization (Docker: LLM sees '/' → actual '/workspace')", () => {

  beforeEach(() => {
    vi.resetModules();
    delete process.env.WORKSPACE_VIRTUAL_ROOT;
    process.env.WORKSPACE_ACTUAL_ROOT = "/workspace";
  });
